...
class Dataset(chainer.dataset.DatasetMixin):
    SOUND_SHAPE = (1, 1, 5 * 44100) #5 seconds

    def __init__(self, root_dir, debug, print_name=False):
        self._paths = glob(os.path.join(root_dir, '*/*.wav'))
        self.labels = {directory.split('/')[-1]:i
    ...
    def get_label_from_path(self, path):   #いらない
        return self.labels[path.split('/')[-2]]
